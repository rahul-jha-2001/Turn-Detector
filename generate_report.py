...
def chart_stage1_language_accuracy():
    df = pd.DataFrame([
        {"language": k, "accuracy": v}
        for k, v in STAGE1_LANG_ACCURACY.items()
    ...
    bars = alt.Chart(df).mark_bar(cornerRadiusEnd=3).encode(
        ...
        color=alt.condition(
            alt.datum.language == "hin",
            alt.value(PRIMARY),
            alt.value(NEUTRAL_LIGHT),
        ),
        tooltip=[
            "language:N",
    ...
    chart = (bars + labels).properties(
        title="Stage-1 Test Accuracy by Language (Hindi highlighted in blue)"
    )
